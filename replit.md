...
- **Auth flows**: Sign-up creates the user, sends a verification email via Resend (no-op if `RESEND_API_KEY` unset), and redirects to `/verify-email?sent=1`. The `/verify-email?token=...` page consumes the token AND mints a single-use `LoginToken` (5-min TTL, SHA-256 hashed at rest); the user then clicks a button that calls `autoSignInAfterVerify()` which invokes `signIn("credentials", { oneTimeToken })` and redirects to `/dashboard`. The Credentials provider accepts EITHER `{email,password}` OR `{oneTimeToken}`; one-time consumption is atomic via `updateMany` with conditional WHERE. Pages: `/sign-in`, `/sign-up`, `/verify-email`, `/forgot-password`, `/reset-password?token=...`, `/admin` (ADMIN/SUPER_ADMIN only). All auth tokens (verify, reset, login) are SHA-256 hashed at rest; password reset always returns `{sent:true}` to prevent enumeration. Password reset bumps `User.passwordChangedAt`; the JWT callback invalidates any session whose `issuedAt` is `<= passwordChangedAt`. Rate-limiting via `withRateLimit()` on signup / signin / forgot. `/dashboard/*` enforces `auth() + emailVerified` in `src/app/dashboard/layout.tsx` (Auth.js v5 JWT decode is not edge-safe with our prisma-using callback, so `src/middleware.ts` is a pass-through reserved for future edge-safe checks).
- **Branding**: matches the reference deployment at `nabk-directory--shoaibhajhussen.replit.app`. Colors: primary `#F2930D` (orange CTA), accent `#1A664D` (deep green), secondary `#E7F6E9` (light green pill), background `#FCFAF8` cream, text `#2E261F`. Fonts: Cairo body + Noto Naskh Arabic for headings (loaded via `next/font/google`). All primary CTAs are pill-shaped; secondary actions use 16px radius. Header logo = white `Store` icon on an orange rounded tile with "دليل النبك" + subtitle "كل ما تحتاجه في مدينتك". Business detail page: when no cover image exists, renders a green→cream→peach diagonal gradient hero containing the category-themed lucide icon (Pill / Stethoscope / Coffee / ShoppingBasket / Wrench / Scissors / GraduationCap / Home / Shirt / Hammer) plus a 80–96px cream rounded-square avatar tile with the business's first Arabic letter — visually identical to the reference site's empty-state pattern.
